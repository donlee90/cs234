...
def value_iteration(P, nS, nA, gamma=0.9, max_iteration=20, tol=1e-3):
    ...
    for i in xrange(max_iteration):
        Vnew = np.zeros(nS)
        for s in xrange(nS):
            Vnew[s] = max(sum(p * (r + gamma * V[sp]) for p, sp, r, terminal in P[s][a])\
                        for a in xrange(nA))

        diff = np.max(np.abs(V - Vnew))
        V = Vnew
    ...
    # Policy extraction
    for s in xrange(nS):
        policy[s] = np.argmax([sum(p * (r + gamma * V[sp]) for p, sp, r, terminal in P[s][a])\
                        for a in xrange(nA)])
...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, max_iteration=1000, tol=1e-3):
    ...
    for i in xrange(max_iteration):
        ...
        for s in xrange(nS):
            a = policy[s]
            Vnew[s] = sum(p * (r + gamma * V[sp]) for p, sp, r, terminal in P[s][a])
