...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(tf.uint8, [None,
                    height, width, nchannels * self.config.state_history])
        self.a = tf.placeholder(tf.int32, [None])
        self.r = tf.placeholder(tf.float32, [None])
        self.sp = tf.placeholder(tf.uint8, [None,
                    height, width, nchannels * self.config.state_history])
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            W = tf.get_variable("W", [s.get_shape()[1], num_actions],
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        trainable = tf.GraphKeys.VARIABLES
        source_vars = tf.get_collection(key=trainable, scope=q_scope)
        target_vars = tf.get_collection(key=trainable, scope=target_q_scope)
        variables = zip(target_vars, source_vars)

        assign_ops = [tf.assign(target, source) for target, source in variables]
    ...
    def add_loss_op(self, q, target_q):
        ...
        q_samp = self.r + self.config.gamma * tf.reduce_max(target_q, axis=1) * not_done

        q_sa = tf.reduce_sum(q * tf.one_hot(indices=self.a, depth=num_actions), axis=1)
        loss = tf.reduce_sum(tf.square(q_samp - q_sa))
        self.loss = loss / self.config.batch_size
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_optimizer_op(self, scope):
        ...
        trainable = tf.GraphKeys.TRAINABLE_VARIABLES
        tvars = tf.get_collection(key=trainable, scope=scope)

        gvs = optimizer.compute_gradients(self.loss, tvars)

        if self.config.grad_clip:
            gvs = [(tf.clip_by_norm(grad, self.config.clip_val), var) for grad,
            var in gvs if grad is not None]
